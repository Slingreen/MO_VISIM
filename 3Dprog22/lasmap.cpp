...
//    int k = new_Points.size();
    ...
    for (float dz = 0; dz < yMax - yMin; dz += ekvidistanse){
        for (float dx = 0; dx < xMax - xMin; dx += ekvidistanse){
            int A{0}, B{0}, C{0}, D{0};
            int NextC{0}, NextB{0};
...
void lasmap::Reduce_Points(float x, float y, float zMin, float e, float* lasth, float xMin, float yMin, int* loop, float xMax){
    ...
    *lasth += sum/count;
    ...
    if (!(x + e >= xMax)){
        ...
        new_Points.push_back(QVector3D(x - xMin, Points[g].z() - zMin, y - yMin)); //fix y values
        return;
    }
}
